...
def parse_args():
    ...
    parser.add_argument(
        "--save", action="store_true", default=False, help="save detection results."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-sid", "--start_index", default=0, type=int, help="start index to test."
    )
    ...
    parser.add_argument(
        "-ct", "--conf_thresh", default=0.1, type=float, help="confidence threshold"
    )
    parser.add_argument(
        "-nt", "--nms_thresh", default=0.5, type=float, help="NMS threshold"
    )
    ...
    # dataset
    parser.add_argument("-d", "--dataset", default="ucf24", help="ucf24, ava.")
    parser.add_argument(
...
@torch.no_grad()
def inference_ucf24_jhmdb21(
    args, model, device, dataset, class_names=None, class_colors=None
):
    # path to save
    if args.save:
        save_path = os.path.join(
            args.save_folder, args.dataset, args.version, "video_clips"
        )
    ...
    for index in range(args.start_index, len(dataset)):
        ...
        if args.save:
            # save result
            cv2.imwrite(
                os.path.join(save_path, "{:0>5}.jpg".format(index)), vis_results
            )
...
def inference_ava(args, model, device, dataset, class_names=None, class_colors=None):
    ...
    for index in range(args.start_index, len(dataset)):
        ...
        if args.save:
            # save result
            cv2.imwrite(os.path.join(save_path, "{:0>5}.jpg".format(index)), key_frame)
