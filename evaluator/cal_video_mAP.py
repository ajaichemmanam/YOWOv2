...
def compute_score_one_class(bbox1, bbox2, w_iou=1.0, w_scores=1.0, w_scores_mul=0.5):
    ...
    for i in range(n_bbox1):
        ...
        for j in range(n_bbox2):
            ...
            scores[i, j] = (
                w_iou * bbox_iou_frames
                + w_scores * sum_score_frames
                + w_scores_mul * mul_score_frames
            )
...
def link_bbxes_between_frames(bbox_list, w_iou=1.0, w_scores=1.0, w_scores_mul=0.5):
    nfr = len(bbox_list)
    ind_notempty = [i for i in range(nfr) if np.array(bbox_list[i]).size]
    ...
    edge_scores = [
        compute_score_one_class(
            detect[i],
            detect[i + 1],
            w_iou=w_iou,
            w_scores=w_scores,
            w_scores_mul=w_scores_mul,
        )
    ...
    while not np.any(isempty_vertex):
        ...
        for i in range(nframes):
            ...
            if i < nframes - 1:
                iouscore = (
                    copy_edge_scores[i][j, idx[i + 1]]
                    - bbox_list[i][j, 4]
                    - bbox_list[i + 1][idx[i + 1], 4]
                )
...
def video_ap_one_class(gt, pred_videos, iou_thresh=0.2, bTemporal=False):
    ...
    for pred_v in pred_videos:
        ...
        pred_link_v = link_video_one_class(
            pred_v[1], True
        )  # [array<frame_index, x1,y1,x2,y2, cls_score>]
        pred.extend((video_index, tube) for tube in pred_link_v)
        ...
        if video_index in gt_v_index:
            ...
            if gt_this:
                if bTemporal:
                    ...
                    iou = np.array(
                        [
                            iou3d(g, boxes[int(g[0, 0] - 1) : int(g[-1, 0]), :5])
                            for g in gt_this
                        ]
                    )
                elif boxes.shape[0] < gt_this[0].shape[0]:
                    ...
                    iou = np.array(
                        [
                            iou3d(
                                g[int(boxes[0, 0] - 1) : int(boxes[-1, 0]), :],
                                boxes[:, :5],
                            )
                            for g in gt_this
                        ]
...
def gt_to_videts(gt_v):
    keys = sorted(gt_v.keys())
    res = []
    for i in range(len(keys)):
        # annotation of the video: tubes and gt_classes
        v_annot = gt_v[keys[i]]
        res.extend(
            [v_annot["gt_classes"], i + 1, v_annot["tubes"][j]]
            for j in range(len(v_annot["tubes"]))
        )
    return res


def evaluate_videoAP(
    gt_videos, all_boxes, num_classes, iou_thresh=0.2, bTemporal=False
):
    """
