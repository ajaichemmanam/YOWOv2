...
class AVA_Evaluator(object):
    def __init__(
        ...
        self.backup_dir = os.path.join(self.backup_dir, f"ava_{version}")
        os.makedirs(self.backup_dir, exist_ok=True)
    ...
    def calculate_mAP(self, epoch):
        ...
        write_results(
            detections, os.path.join(self.backup_dir, f"detections_{epoch}.csv")
        )
        write_results(
            groundtruth, os.path.join(self.backup_dir, f"groundtruth_{epoch}.csv")
        )
        ...
        print(f"Save eval results in {self.output_json}")
            ...
            with torch.no_grad():
                ...
                for bi in range(len(batch_bboxes)):
                    ...
                    for bbox in bboxes:
                        ...
                        cls_out = [
                            np.sqrt(det_conf * cls_conf) for cls_conf in bbox[5:]
                        ]
