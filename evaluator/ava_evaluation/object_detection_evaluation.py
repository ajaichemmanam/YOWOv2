...
class ObjectDetectionEvaluator(DetectionEvaluator):
    ...
    def __init__(
        ...
        self._metric_prefix = f"{metric_prefix}_" if metric_prefix else ""
    ...
    def evaluate(self):
        ...
        pascal_metrics = {
            f"{self._metric_prefix}Precision/mAP@{self._matching_iou_threshold}IOU": mean_ap
        }
        if self._evaluate_corlocs:
            pascal_metrics[
                f"{self._metric_prefix}Precision/meanCorLoc@{self._matching_iou_threshold}IOU"
            ] = mean_corloc
        category_index = label_map_util.create_category_index(self._categories)
        for idx in range(per_class_ap.size):
            if idx + self._label_id_offset in category_index:
                display_name = f'{self._metric_prefix}PerformanceByCategory/AP@{self._matching_iou_threshold}IOU/{category_index[idx + self._label_id_offset]["name"]}'
                pascal_metrics[display_name] = per_class_ap[idx]

                # Optionally add CorLoc metrics.classes
                if self._evaluate_corlocs:
                    display_name = f'{self._metric_prefix}PerformanceByCategory/CorLoc@{self._matching_iou_threshold}IOU/{category_index[idx + self._label_id_offset]["name"]}'
