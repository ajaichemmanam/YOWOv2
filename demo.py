...
def multi_hot_vis(args, frame, out_bboxes, orig_w, orig_h, class_names, act_pose=False):
    ...
    for bbox in out_bboxes:
        x1, y1, x2, y2 = bbox[:4]
        cls_conf = (
            bbox[5 : 5 + 14] if act_pose else bbox[5:]
        )  # only show 14 poses of AVA else # show all actions of AVA.
...
def detect(args, model, device, transform, class_names, class_colors):
    ...
    while True:
        ...
        # prepare
        if len(video_clip) <= 0:
            video_clip.extend(frame_pil for _ in range(args.len_clip))
