...
def run(args, d_cfg, model, device, transform, class_names):
    ...
    while True:
        ...
        # prepare
        if len(video_clip) <= 0:
            video_clip.extend(frame_pil for _ in range(d_cfg["len_clip"]))
        ...
        for bbox in bboxes:
            x1, y1, x2, y2 = bbox[:4]
            cls_out = [float(bbox[4]) * cls_conf.cpu().numpy() for cls_conf in bbox[5]]
            ...
            if scores := list(scores):
                ...
                for _, cls_ind in enumerate(indices):
                    ...
                    text_size.append(
                        cv2.getTextSize(text[-1], font, fontScale=0.25, thickness=1)[0]
                    )
