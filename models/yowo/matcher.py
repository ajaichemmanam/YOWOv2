...
class SimOTA(object):
    ...
    def __call__(
        ...
        strides = torch.cat(
            [
                torch.ones_like(anchor_i[:, 0]) * stride_i
                for stride_i, anchor_i in zip(fpn_strides, anchors)
            ],
            dim=-1,
        )
        # List[F, M, 2] -> [M, 2]
        ...
        fg_mask, is_in_boxes_and_center = self.get_in_boxes_info(
            tgt_bboxes, anchors, strides, num_anchor, num_gt
        )
        ...
        ) = self.dynamic_k_matching(cost, pair_wise_ious, tgt_labels, num_gt, fg_mask)
    ...
    def get_in_boxes_info(
        ...
        gt_bboxes_l = (
            gt_centers[:, 0].unsqueeze(1).repeat(1, num_anchors) - center_radius_
        )  # x1
        gt_bboxes_t = (
            gt_centers[:, 1].unsqueeze(1).repeat(1, num_anchors) - center_radius_
        )  # y1
        gt_bboxes_r = (
            gt_centers[:, 0].unsqueeze(1).repeat(1, num_anchors) + center_radius_
        )  # x2
