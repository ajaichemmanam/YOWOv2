...
class YOWO(nn.Module):
    def __init__(
        ...
        ## head
        self.heads = nn.ModuleList([build_head(cfg) for _ in range(len(cfg["stride"]))])

        ## pred
        head_dim = cfg["head_dim"]
        self.conf_preds = nn.ModuleList(
            [nn.Conv2d(head_dim, 1, kernel_size=1) for _ in range(len(cfg["stride"]))]
        )
        ...
        self.reg_preds = nn.ModuleList(
            [nn.Conv2d(head_dim, 4, kernel_size=1) for _ in range(len(cfg["stride"]))]
        )
    ...
    def post_process_one_hot(self, conf_preds, cls_preds, reg_preds, anchors):
        ...
        for level, (conf_pred_i, cls_pred_i, reg_pred_i, anchors_i) in enumerate(
            zip(conf_preds, cls_preds, reg_preds, anchors)
        ):
            # (H x W x C,)
            scores_i = (
                torch.sqrt(conf_pred_i.sigmoid() * cls_pred_i.sigmoid())
            ).flatten()
    ...
    def inference(self, video_clips):
        ...
        for level, (cls_feat, reg_feat) in enumerate(zip(cls_feats, reg_feats)):
            ...
            cls_pred = (
                cls_pred.permute(0, 2, 3, 1).contiguous().view(B, -1, self.num_classes)
            )
        ...
        if self.multi_hot:
            ...
            for batch_idx in range(video_clips.size(0)):
                ...
                cur_cls_preds = []
                cur_reg_preds = []
                for conf_preds, cls_preds, reg_preds in zip(
                    all_conf_preds, all_cls_preds, all_reg_preds
                ):
                    # [B, M, C] -> [M, C]
                ...
                out_boxes[..., :4] = out_boxes[..., :4].clip(0.0, 1.0)
