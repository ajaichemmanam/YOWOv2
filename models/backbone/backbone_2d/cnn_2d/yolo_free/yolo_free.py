...
class FreeYOLO(nn.Module):
    def __init__(self, cfg):
        ...
        self.neck = build_neck(
            cfg=self.cfg, in_dim=bk_dim[-1], out_dim=self.cfg["neck_dim"]
        )

        ## fpn
        self.fpn = build_fpn(
            cfg=self.cfg, in_dims=self.cfg["fpn_dim"], out_dim=self.cfg["head_dim"]
        )
...
def build_yolo_free(model_name="yolo_free_large", pretrained=False):
    ...
    if pretrained:
        ...
        if url is None:
            ...
            print(f"Loading 2D backbone pretrained weight: {model_name.upper()}")
