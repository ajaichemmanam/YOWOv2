...
def get_ious(bboxes1, bboxes2, box_mode="xyxy", iou_type="iou"):
    ...
    if iou_type == "iou":
        ...
        g_w_intersect = torch.max(bboxes1[..., 2], bboxes2[..., 2]) - torch.min(
            bboxes1[..., 0], bboxes2[..., 0]
        )
        g_h_intersect = torch.max(bboxes1[..., 3], bboxes2[..., 3]) - torch.min(
            bboxes1[..., 1], bboxes2[..., 1]
        )
...
def rescale_bboxes(bboxes, orig_size):
    ...
    bboxes[..., [0, 2]] = np.clip(bboxes[..., [0, 2]] * orig_w, a_min=0.0, a_max=orig_w)
    bboxes[..., [1, 3]] = np.clip(bboxes[..., [1, 3]] * orig_h, a_min=0.0, a_max=orig_h)
